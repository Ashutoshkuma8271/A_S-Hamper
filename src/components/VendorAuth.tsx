...
import { Store, Mail, Lock, UserRound, Phone, FileText, Hash, ArrowRight, Loader2, CheckCircle2, Sparkles, ShieldCheck } from 'lucide-react';
...
export default function VendorAuth() {
  ...
  const [mode, setMode] = useState<Mode>('login'); // Default: 'login' as requested
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-sm text-[#7F011F]/70 dark:text-[#F5EBD0]/70">
              ...
              <span className="capitalize font-semibold">{profile?.role || 'vendor'}</span>
            </p>
          </div>
        </div>
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (mode === 'signup') {
        ...
        toast.success('Vendor registered! Please enter verification code.');
        ...
        toast.success('Signed in to Vendor Portal successfully!');
    ...
    return (
      <div className="rounded-3xl bg-[#FAF6EB] dark:bg-[#240008] border border-[#7F011F]/20 p-8 sm:p-10 text-center shadow-xl">
        ...
        <p className="mt-2 text-sm text-[#7F011F]/70 dark:text-[#F5EBD0]/70 max-w-sm mx-auto">
          Your vendor account is ready. Sign in with your registered email and password to access your Vendor Studio.
        </p>
        <button
          onClick={() => {
            setDone(false);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-3xl bg-[#FAF6EB] dark:bg-[#240008] border border-[#7F011F]/20 p-6 sm:p-8 lg:p-10 shadow-[0_20px_60px_-20px_rgba(127,1,31,0.15)]">
        ...
        <form onSubmit={handleSubmit} autoComplete="off" className="space-y-4">
          {mode === 'signup' && (
            <Field icon={<UserRound className="h-4 w-4" />} label="Business / Brand Name">
              <input
                required
                autoComplete="off"
                value={businessName}
                onChange={(e) => setBusinessName(e.target.value)}
                placeholder="e.g. Royal Delights Hamper Co."
          ...
          <Field icon={<Mail className="h-4 w-4" />} label="Business Email Address">
            <input
              type="email"
              required
              autoComplete="off"
          ...
          <Field icon={<Lock className="h-4 w-4" />} label="Password">
            <PasswordField
              ...
              placeholder={mode === 'signup' ? 'Create a secure password' : 'Enter your vendor password'}
            />
          </Field>
          ...
          {mode === 'signup' && (
            <div className="space-y-4 pt-1">
              <div className="grid sm:grid-cols-2 gap-4">
                <Field icon={<Hash className="h-4 w-4" />} label="Shop / Studio ID">
                  <input
                    required
                    value={shopNo}
                    onChange={(e) => setShopNo(e.target.value)}
                    placeholder="STUDIO-01"
                    className="w-full h-11 rounded-xl border border-[#7F011F]/30 bg-white dark:bg-[#180005] px-3.5 text-sm text-[#7F011F] dark:text-[#F5EBD0] outline-none focus:border-[#7F011F]"
                  />
                </Field>
                <Field icon={<FileText className="h-4 w-4" />} label="GSTIN / Business Reg No">
                  <input
                    required
                    value={gstNo}
              ...
              <Field icon={<Phone className="h-4 w-4" />} label="Contact Mobile Number">
                <CountryPhoneInput
              ...
              <Field icon={<Store className="h-4 w-4" />} label="Studio / Dispatch Address">
                <input
                  required
                  value={businessAddress}
                  onChange={(e) => setBusinessAddress(e.target.value)}
                  placeholder="Street / Warehouse / Studio Address"
                  className="w-full h-11 rounded-xl border border-[#7F011F]/30 bg-white dark:bg-[#180005] px-3.5 text-sm text-[#7F011F] dark:text-[#F5EBD0] outline-none focus:border-[#7F011F]"
                />
              </Field>
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-3">
                <Field icon={<Store className="h-4 w-4" />} label="City">
                  <input
                    ...
                    placeholder="City"
                    className="w-full h-10 rounded-xl border border-[#7F011F]/30 bg-white dark:bg-[#180005] px-3 text-xs text-[#7F011F] dark:text-[#F5EBD0] outline-none focus:border-[#7F011F]"
                  />
                </Field>
                ...
                <Field icon={<Store className="h-4 w-4" />} label="PIN Code">
                  <input
                    ...
                    className="w-full h-10 rounded-xl border border-[#7F011F]/30 bg-white dark:bg-[#180005] px-3 text-xs font-mono text-[#7F011F] dark:text-[#F5EBD0] outline-none focus:border-[#7F011F]"
                  />
                </Field>
              </div>
            </div>
          ...
          >
            {loading ? (
              ...
              <>
                {mode === 'signup' ? 'Complete Vendor Registration' : 'Sign In to Vendor Studio'}
      ...
      {/* Prominent Below-Card Switch Section (As Requested) */}
      {mode === 'login' ? (
        <div className="rounded-3xl bg-[#F5EBD0] dark:bg-[#180005] border border-[#7F011F]/30 p-6 sm:p-7 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
          <div className="space-y-1 text-center sm:text-left">
            ...
            <p className="text-xs text-[#7F011F]/70 dark:text-[#F5EBD0]/70 max-w-md">
              Start selling curated artisan gifts, build custom hampers, and fulfill customer orders nationwide.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="rounded-3xl bg-[#F5EBD0] dark:bg-[#180005] border border-[#7F011F]/30 p-5 text-center shadow-sm">
          <p className="text-xs sm:text-sm text-[#7F011F]/80 dark:text-[#F5EBD0]/80">
            Already registered as an A_S Hamper partner?{' '}
            <button
              type="button"
              onClick={() => {
                setMode('login');
